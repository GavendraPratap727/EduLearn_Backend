...
using (var scope = app.Services.CreateScope())
{
    try 
    {
        ...
        try {
            Console.WriteLine("Force Reset [V7]: Wiping ReviewService tables and history...");
            dbContext.Database.ExecuteSqlRaw("DROP TABLE IF EXISTS \"Reviews\" CASCADE;");
            dbContext.Database.ExecuteSqlRaw("DROP TABLE IF EXISTS \"__ReviewMigrationsHistory\" CASCADE;");
            Console.WriteLine("ReviewService table wipe successful.");
        } catch (Exception ex) { 
